...
# Tkinter setup
window = tk.Tk()
window.title("UCS Parking Lot Navigation")
...
def start_ucs_to_parking():
    draw_grid()
    global parking_spot, parking_path, nodes_visited_to_parking
    parking_spot, parking_path, nodes_visited_to_parking = ucs(entrance, parking_spots)
...
def start_ucs_to_exit():
    if parking_spot:
        draw_grid()
        update_cell(parking_spot[0], parking_spot[1], PATH_COLOR)  # Mark the current parking spot
        _, path_to_exit, nodes_visited_to_exit = ucs(parking_spot, [exit_point])
...
start_button = tk.Button(window, text="Start UCS to Parking Spot", command=start_ucs_to_parking)
start_button.pack()

start_exit_button = tk.Button(window, text="Start UCS to Exit", command=start_ucs_to_exit, state="disabled")
