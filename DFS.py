...
# Tkinter setup
window = tk.Tk()
window.title("DFS Parking Lot Navigation")
...
def dfs(start, goals):
    ...
    while stack:
        ...
        if current != start and current not in goals:
            ...
            time.sleep(0.15)  # Slower delay for better observation
        ...
        # Explore neighbors
        for (d_row, d_col) in [(0, -1), (0, 1), (-1, 0), (1, 0)]:  # Left, Right, Up, Down
...
def start_dfs_to_parking():
    draw_grid()
    global parking_spot, parking_path, nodes_visited_to_parking
    parking_spot, parking_path, nodes_visited_to_parking = dfs(entrance, parking_spots)
...
def start_dfs_to_exit():
    if parking_spot:
        draw_grid()
        update_cell(parking_spot[0], parking_spot[1], PATH_COLOR)  # Mark the current parking spot
        _, path_to_exit, nodes_visited_to_exit = dfs(parking_spot, [exit_point])
...
start_button = tk.Button(window, text="Start DFS to Parking Spot", command=start_dfs_to_parking)
start_button.pack()

start_exit_button = tk.Button(window, text="Start DFS to Exit", command=start_dfs_to_exit, state="disabled")
